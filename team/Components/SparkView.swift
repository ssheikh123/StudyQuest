...
struct SparkView: View {
    ...
    init(state: Mood, size: CGFloat = 120, reduceMotion: Bool = false) {
        self.init(mood: state, size: size, reduceMotion: reduceMotion)
    }
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            Timer.scheduledTimer(withTimeInterval: 3.4, repeats: true) { _ in
                isBlinking = true
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.16) { isBlinking = false }
            }
        }
        ...
        ZStack {
            HStack(spacing: size * 0.18) {
                RoundedRectangle(cornerRadius: size * 0.04)
                    .fill(.white)
                    .frame(width: size * 0.09, height: (isBlinking || mood == .sleeping) ? size * 0.018 : size * 0.09)
                RoundedRectangle(cornerRadius: size * 0.04)
                    .fill(.white)
                    .frame(width: size * 0.09, height: (isBlinking || mood == .sleeping) ? size * 0.018 : size * 0.09)
            }
    ...
    private var belly: some View {
        Capsule()
            .fill(AppTheme.goldReward.opacity(0.82))
    ...
    private var tail: some View {
        Capsule()
            .fill(LinearGradient(colors: [AppTheme.purpleAccent, Color.cyan], startPoint: .leading, endPoint: .trailing))
    ...
    private var moodSymbol: some View {
        ...
        case .thinking:
            Image(systemName: "questionmark.circle.fill")
                ...
                .offset(x: size * 0.38, y: -size * 0.32)
        case .sleeping:
            Text("Z")
                .font(.system(size: size * 0.18, weight: .bold, design: .rounded))
                .foregroundStyle(Color.cyan)
                .offset(x: size * 0.34, y: -size * 0.38)
        case .happy:
